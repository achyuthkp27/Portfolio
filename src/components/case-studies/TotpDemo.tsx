...
const TotpDemo = () => {
  ...
  return (
    <div className="w-full max-w-md">
      <div className="flex justify-center gap-1.5 md:gap-2 mb-3" aria-label={`Current code ${code.split("").join(" ")}`}>
        {code.split("").map((digit, i) => (
          <span
            key={i}
            className="w-10 h-12 md:w-11 md:h-14 rounded-md bg-white/[0.05] border border-white/20 flex items-center justify-center font-mono text-xl md:text-2xl text-emerald-100 tabular-nums"
          >
            {digit}
          </span>
        ))}
      ...
      <div className="h-1 rounded-full bg-white/10 overflow-hidden mb-2" aria-hidden="true">
        <div
      ...
      <p className="text-center font-mono text-[11px] text-white/55 mb-5">
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <button
          type="button"
          onClick={submit}
          className="px-3 py-2 rounded-md text-xs md:text-[13px] font-body font-medium border border-emerald-400/40 text-emerald-200 hover:bg-emerald-400/10 transition-colors"
        >
          Submit this code
        </button>
        <button
          type="button"
          onClick={submitWrong}
          className="px-3 py-2 rounded-md text-xs md:text-[13px] font-body font-medium border border-white/15 text-white/80 hover:bg-white/5 transition-colors"
        >
          Submit a wrong code
        </button>
      </div>
      ...
      <p
        className={`min-h-[40px] rounded-lg border border-white/10 bg-black/40 px-3 py-2.5 font-mono text-[11px] leading-relaxed ${
          verdict ? (verdict.tone === "ok" ? "text-emerald-300" : "text-amber-200") : "text-white/45"
        }`}
        aria-live="polite"
      >
        {verdict ? verdict.text : "Submit the code twice to see replay prevention."}
      </p>
      <p className="mt-3 text-[11px] font-body text-white/45">Real RFC 6238 running in your browser, with a demo secret</p>
    </div>
  );
};
